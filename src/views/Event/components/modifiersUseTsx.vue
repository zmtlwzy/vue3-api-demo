      ...
      <>
        <div class="column-layout">
          <label>
            ...
            <input type="text" value={props.customName} onInput={(e: ChangeEvent) => handleChange(e.target.value)} />
          </label>
        </div>
      </>
    );
...
export const vModelCom2 = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return (): JSX.Element => (
      <div>
        <input type="number" value={props.modelValue} onInput={(e: ChangeEvent) => handleChange(e.target.value)} />
      </div>
    );
  }
});
